...
float* calculateFlow(string files[], bool verbose = false)
{ 
	cv::Mat tU;
	cv::Mat tV;
...
{ 
	...
	ofstream outfile ("aa.txt",ofstream::binary);
	...
	unsigned int const  sw  = 2;
	unsigned int const  sdw = 4;

	outfile.write(&sig1, sizeof(sig1));
	outfile.write(&sig2, sizeof(sig2));
	outfile.write(cdw, sdw);
	outfile.write(cw , sw );
	outfile.write(cw , sw );
	outfile.write(cdw, sdw);
	outfile.close();
	if (true){
		string file[3] = {"Data\\RubberWhale\\flow10.flo", "Data\\RubberWhale\\frame10.png", "Data\\RubberWhale\\frame11.png"};
		calculateFlow(file, true);
	}else{
		string files[8][3];
