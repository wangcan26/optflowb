...
void middlebury::ReadFlowFile(middlebury::CFloatImage& img, const char* filename)
{
    if (filename == NULL)
    ...
    int nBands = 2;
	middlebury::CShape sh(width, height, nBands);
    img.ReAllocate(sh);
...
void middlebury::WriteFlowFile(middlebury::CFloatImage img, const char* filename)
{
    if (filename == NULL)
    ...
    middlebury::CShape sh = img.Shape();
